    ...
    <div class="packs">
        ...
        <div class="row mx-0 mb-5 p-0 justify-content-center justify-content-md-start">
            <div class="col-12">
                <!-- ADD PROMOTIONS -->
                <transition name="fade">
                    <form class="card card-body my-5" @submit.prevent="AddPromo" v-if="adding">
                        <h4 class="mb-4">Nueva Promoción</h4>
        ...
        <div class="packs-cards row mt-0 mb-5 p-0 justify-content-center justify-content-md-start" v-if="promoSaves.length != 0">
            <div class="col-10 col-sm-8 col-lg-6 col-xl-4 p-1 my-5 my-sm-0" v-for="(promo, index) in promoSaves" :key="index">
                <div class="card promo">
                    <div class="card-body">
                        ...
                        <hr>
                          <img src="/img/shop/promo1.png" class="img-fluid d-block mx-auto">
                        <hr>

                        <p><b>Descripción: </b>{{ promo.description }}</p>

                        <hr>

                        <p><b>Duración: </b>{{ promo.duration }}</p>

                        <hr>

                        <p><b>Vence el: </b>{{ promo.expiration }}</p>

                        <hr>

                        <p class="price"><b>Precio: </b>{{ promo.price }}$</p>

                        <hr>

                        <div class="d-flex justify-content-center">
                            <a class="btn btn-warning" data-target="#modal-edit" @click="EditPromo(promo)">Editar</a>
                            <a class="btn btn-danger" @click="RemovePromo(promo, index)">Eliminar</a>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <transition name="fade" v-if="editing">
            <form class="modal-edit edit2" id="modal-edit" @submit.prevent="SaveEditPromo(newPromo)">
                <h5 class="text-center d-block font-weight-bold">Editar Promoción</h5>
                <hr class="mt-1 mb-3">
                <div class="form-label-group">
                    <input
                        type="text"
                ...
                <div class="form-label-group">
                    <input
                        ...
                        v-model="newPromo.expiration"
                    />
                    <label for="expiration">Vence el</label>
                </div>
...
export default {
    ...
    methods: {
          ...
          if (this.newPromo.name.trim() === '' || this.newPromo.description.trim() === '' || this.newPromo.duration.trim() === '' || this.newPromo.price.trim() === '' || isNaN(this.newPromo.price)) {
            ...
            return;
              ...
              const params = {
                ...
                duration: this.newPromo.duration,
                expiration: this.newPromo.expiration,
                price: this.newPromo.price,
              } 
      
              this.newPromo = {name: '', description: '', duration: '', expiration: '', price: ''};   
              
              axios.post('/promotions', params)
              .then((res) => {
                const response = res.data;
                this.promoSaves.push(response);
                this.showMessage = true;
                this.classSave = "alert-success";
                this.serverMessage = "Promoción Creada Sastisfactoriamente !";
                this.adding = false;
                this.msgTimer = setInterval(() => {
                  this.showMessage = false;
                }, 4000);
        ...
        EditPromo(item) {
            ...
            this.newPromo.description = item.description;
            this.newPromo.duration = item.duration;
            this.newPromo.expiration = item.expiration;
            this.newPromo.price = item.price;
        },
        SaveEditPromo(item) {
            clearInterval(this.msgTimer)
            if (this.newPromo.name.trim() === '' || this.newPromo.description.trim() === '' || this.newPromo.duration.trim() === '' || this.newPromo.price.trim() === '' || isNaN(this.newPromo.price)) {
                ...
                axios.put(`/promotions/${item.id}`, params)
                .then(res => {
                    const idx = this.promoSaves.findIndex(search => search.id === res.data.id);
                    this.promoSaves[idx] = res.data;
                    this.showMessage = true;
        ...
        RemovePromo(item, index) {
            axios.delete(`/promotions/${item.id}`)
            .then(() => {
                clearInterval(this.msgTimer)
                this.promoSaves.splice(index, 1);
                this.showMessage = true;
                this.serverMessage = "Promoción Eliminada Sastisfactoriamente !";
                this.classSave = "alert-danger";
                this.msgTimer = setInterval(() => {
                this.showMessage = false;
                }, 4000);
